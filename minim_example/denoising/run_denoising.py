...
for data_path in ['baboon_gray.png','boat.png','Cameraman.jpg','house.tif','jetplane.tif','lake.tif','livingroom.tif']:
    for noise_parameter in [5,10,15,20]:
        parameters = {}
        net_list = []
        size = 256
        gpu_id = 0
        ...
        parameters['data_p'] = {'data_shape':(size,size),'random_rate':0.,
                                'pre_full':True,'mask_type':mask_type,'mask_path':'../mask/mask.png',
                                'data_path':'../img/'+data_path,'data_type':'gray_img','ymode':'denoising',
                               'noise_mode':'gaussian','noise_parameter':noise_parameter}

        parameters['train_p'] = {'train_epoch':30000}
        ...
        parameters['reg_p'] = {'reg_name':'MultiReg','reg_list':[
            {'reg_name':'INRR','coef':1e-2,'n':size,'mode':0,
            ...
            {'reg_name':'INRR','coef':1e-2,'n':size,'mode':1,
             'w0_initial':1.,'lap_k':lap_k,'lap_mode':'Huber',
             'huber_delta':huber_delta,'inrr_alpha':inrr_alpha,
             'nabla_matrix_order_k':nabla_matrix_order_k,
             'inr_parameter':{'dim_in': 1,'dim_out':100,'w0_initial':20.}}]}
