...
class rssnet(object):
    ...
    def train(self,get_x=None,verbose=True):
        ...
        if self.data_p['return_data_type'] in ['tensor','random']:
            ...
            for ite in range(self.train_p['train_epoch']):
                ...
                if (self.net_p['net_name'] in ['UNet','ResNet','skip']) or (self.net_p['net_name']=='KNN' and self.net_p['mode'] in ['UNet','ResNet','skip']) or \
                    (self.net_p['net_name']=='composition' and self.net_p['net_list'][0]['mode'] in ['UNet','ResNet','skip']):
    ...
    def cal_nmae(self,pre, target):
        ...
        if unseen_num<1e-3:
            ...
            return t.sum(t.abs((pre-target)*(1-self.mask)))/unseen_num/(max_pixel-min_pixel)
        pass
