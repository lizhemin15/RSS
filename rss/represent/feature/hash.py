...
class HashEmbedders(nn.Module):
    ...
    def linear_interp(self, x, voxel_min_vertex, voxel_max_vertex, voxel_embedds):
        weights = (x - voxel_min_vertex) / (voxel_max_vertex - voxel_min_vertex)  # B x 1
        c0 = voxel_embedds[:, 0] * (1 - weights) + voxel_embedds[:, 1] * weights
        return c0
        ...
        if self.non_local_if == False or self.forward_count < 100:
            if x.shape[1] == 3:
                ...
                for i in range(self.n_levels):
                    ...
                    x_embedded = self.linear_interp(x, voxel_min_vertex, voxel_max_vertex, voxel_embedds)
                    x_embedded_all.append(x_embedded)
